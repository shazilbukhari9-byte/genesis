...
def create_cert():
    ...
    cur.execute(
        ...
        (g.tenant_id, g.user_name, 'Certificate uploaded', name),
    )
    conn.commit()
    conn.close()
...
def update_cert(cert_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (tenant_id, who, action, detail, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.tenant_id, g.user_name, 'Certificate updated', row['name']),
    )
    conn.commit()
    conn.close()
    return jsonify(row)
...
def delete_cert(cert_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (tenant_id, who, action, detail, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.tenant_id, g.user_name, 'Certificate deleted', existing['name']),
    )
    conn.commit()
    conn.close()
