...
def create_canned():
    ...
    cur.execute(
        ...
        (g.tenant_id, g.user_name, 'Canned response created', name),
    )
    conn.commit()
    conn.close()
...
def update_canned(canned_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (tenant_id, who, action, detail, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.tenant_id, g.user_name, 'Canned response updated', name),
    )
    conn.commit()
    conn.close()
...
def delete_canned(canned_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (tenant_id, who, action, detail, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.tenant_id, g.user_name, 'Canned response deleted', existing['name']),
    )
    conn.commit()
    conn.close()
