...
def create_contact_list():
    ...
    cur.execute(
        ...
        (g.user_name, 'Create contact list', name, g.tenant_id),
    )
    conn.commit()
    conn.close()
...
def update_contact_list(list_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (who, action, detail, tenant_id, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.user_name, 'Update contact list', row['name'], g.tenant_id),
    )
    conn.commit()
    conn.close()
    return jsonify(row)
...
def delete_contact_list(list_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (who, action, detail, tenant_id, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.user_name, 'Delete contact list', existing['name'], g.tenant_id),
    )
    conn.commit()
    conn.close()
...
def add_contact(list_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (who, action, detail, tenant_id, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.user_name, 'Add contact', f'{list_id} › {phone}', g.tenant_id),
    )
    conn.commit()
    conn.close()
    return jsonify(row), 201
...
def import_contacts(list_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (who, action, detail, tenant_id, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.user_name, 'Import contacts', f'{list_id}: {ok} added, {len(fail)} rejected', g.tenant_id),
    )
    conn.commit()
    conn.close()
...
def mark_contact_dnc(list_id, contact_id):
    ...
    cur.execute(
        'INSERT INTO audit_log (who, action, detail, tenant_id, created_at) VALUES (%s,%s,%s,%s, now())',
        (g.user_name, 'Mark DNC', row['data'].get('Phone', ''), g.tenant_id),
    )
    conn.commit()
    conn.close()
    return jsonify(row)
