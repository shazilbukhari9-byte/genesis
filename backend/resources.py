...
REGISTRY = {
    ...
    "scripts": dict(
        table="scripts",
        order="name",
        # content is the editor's full page/component/variable tree
        # ({pages, comps, vars}, mirroring the frontend's window.SCR) —
        # a dict, so it round-trips through psycopg2's global dict->Json
        # adapter (db.py) same as flows.graph, no json_fields needed.
        fields=["tenant_id", "name", "type", "published", "content", "ver"],
        search=["name"],
        perm=None,
    ),
