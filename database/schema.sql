...
-- prevents two routes from matching the exact same number the same way
...
-- Admin > Routing > Emergency Groups. flows is a snapshot list of ids
-- rather than a normalised join (matches call_routes/queues' own bare-
-- column convention for this app). members is a plain array of
-- users.id — this table doesn't need a join table since a member is
-- just "notify this person", not a relationship with its own attributes.
-- emergency_contacts/notification_rules/escalation_tiers are each a
-- JSONB list of small structured entries (contact card / channel toggle
-- / {after_minutes, channels}) that only this group ever reads — same
-- reasoning as flows.graph or prompts' earlier audio columns: a real
-- column, not a join table, because nothing else in the schema needs to
-- query into it.
...
-- content holds the editor's full page/component/variable tree (the
-- same shape as the frontend's window.SCR: {pages, comps, vars}) —
-- previously only name/type/published persisted and the canvas
-- (scriptView()) was visual-only; ver is a plain publish counter,
-- client-incremented on Publish same as every other client-trusted
-- field in this generic-REGISTRY CRUD (e.g. emergency_groups.active).
...
ALTER TABLE scripts ADD COLUMN IF NOT EXISTS ver INTEGER NOT NULL DEFAULT 0;
